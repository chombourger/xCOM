...
#define TRACE_STRINGIFY(x) _TRACE_STRINGIFY(x)
...
extern void trace_start (const char *klass, const char *file, int line, const char *func);
extern void trace (const char *fmt, ...);
extern void trace_end ();

#define TRACE1(x) do {							\
   if (TRACE_CONCAT2 (__TRACE_CLASS_,TRACE_CLASS_DEFAULT) >= 1) {	\
      trace_start (TRACE_STRINGIFY (TRACE_CLASS_DEFAULT), __FILE__, __LINE__, __PRETTY_FUNCTION__);		\
      trace x;								\
      trace_end ();							\
   }									\
} while (0)
